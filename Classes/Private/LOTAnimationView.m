...
#if TARGET_OS_IPHONE || TARGET_OS_SIMULATOR
    KWLotieDefaultImageCache *cache = [[KWLotieDefaultImageCache alloc] init];
    if ([cache conformsToProtocol:@protocol(LOTImageCache)]) {
        [KWLOTDefaultCacheProvider setDefaultImageCache:cache];
    }
#endif
...
- (void)_restoreState {
  if (_isAnimationPlaying) {
    ...
    if (_playRangeStartFrame && _playRangeEndFrame) {
      [self playFromFrame:_playRangeStartFrame.floatValue toFrame:_playRangeEndFrame.floatValue withCompletion:self.completionBlock];
...
- (CGFloat)_progressForFrame:(CGFloat)frame {
  if (!_sceneModel) {
    return 0;
  }
  return ((frame - _sceneModel.startFrame.floatValue) / (_sceneModel.endFrame.floatValue - _sceneModel.startFrame.floatValue));
}

- (CGFloat)_frameForProgress:(CGFloat)progress {
  if (!_sceneModel) {
    return 0;
  }
...
- (void)play {
  ...
  [self playFromFrame:_sceneModel.startFrame.floatValue toFrame:_sceneModel.endFrame.floatValue withCompletion:nil];
}
...
- (void)playWithCompletion:(LOTAnimationCompletionBlock)completion {
  ...
  [self playFromFrame:_sceneModel.startFrame.floatValue toFrame:_sceneModel.endFrame.floatValue withCompletion:completion];
}
...
- (void)playToFrame:(CGFloat)toFrame
     withCompletion:(nullable LOTAnimationCompletionBlock)completion {
  [self playFromFrame:_sceneModel.startFrame.floatValue toFrame:toFrame withCompletion:completion];
}

- (void)playFromFrame:(CGFloat)fromStartFrame
              toFrame:(CGFloat)toEndFrame
       withCompletion:(nullable LOTAnimationCompletionBlock)completion {
  if (_isAnimationPlaying) {
    return;
  }
  ...
  BOOL playingForward = ((_animationSpeed > 0) && (toEndFrame > fromStartFrame))
    || ((_animationSpeed < 0) && (fromStartFrame > toEndFrame));

  CGFloat leftFrameValue = MIN(fromStartFrame, toEndFrame);
  CGFloat rightFrameValue = MAX(fromStartFrame, toEndFrame);

  CGFloat currentFrame = [self _frameForProgress:_animationProgress];

  currentFrame = MAX(MIN(currentFrame, rightFrameValue), leftFrameValue);

  if (currentFrame == rightFrameValue && playingForward) {
    currentFrame = leftFrameValue;
  } else if (currentFrame == leftFrameValue && !playingForward) {
  ...
  if (!self.window) {
    ...
    NSTimeInterval duration = (ABS(toEndFrame - fromStartFrame) / _sceneModel.framerate.floatValue);
...
- (void)stop {
  _isAnimationPlaying = NO;
  if (_sceneModel) {
    [self setProgressWithFrame:_sceneModel.startFrame.floatValue callCompletionIfNecessary:YES];
  }
}
...
- (void)pause {
  ...
  CGFloat frame = _compContainer.presentationLayer.currentFrame;
...
- (void)setProgressWithFrame:(CGFloat)currentFrame {
  [self setProgressWithFrame:currentFrame callCompletionIfNecessary:YES];
}

- (void)setProgressWithFrame:(CGFloat)currentFrame callCompletionIfNecessary:(BOOL)callCompletion {
  [self _removeCurrentAnimationIfNecessary];
...
//  [_compContainer displayWithFrame:_compContainer.currentFrame forceUpdate:YES];
    [_compContainer displayWithFloatFrame:_compContainer.currentFrame forceUpdate:YES];
  [CATransaction commit];
}
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)complete {
  if ([_compContainer animationForKey:kCompContainerAnimationKey] == anim &&
    ...
    if (complete) {
      ...
      frame = ([self _isSpeedNegative] ? (NSNumber *)playAnimation.toValue : (NSNumber *)playAnimation.fromValue).floatValue;
    }
    [self _removeCurrentAnimationIfNecessary];
    [self setProgressWithFrame:frame callCompletionIfNecessary:NO];
