...
- (BOOL)hasUpdateForFrame:(CGFloat)frame {
  ...
  if (self.leadingKeyframe &&
      ...
      self.leadingKeyframe.keyframeTime.floatValue < frame) {
    // Frame is after bounds of keyframes. Clip
    return NO;
  }
  if (self.trailingKeyframe &&
      self.leadingKeyframe == nil &&
      self.trailingKeyframe.keyframeTime.floatValue > frame) {
...
- (void)updateKeyframeSpanForFrame:(CGFloat)frame {
  ...
  if (self.trailingKeyframe && frame >= self.trailingKeyframe.keyframeTime.floatValue) {
  ...
  } else if (self.leadingKeyframe && frame < self.leadingKeyframe.keyframeTime.floatValue) {
...
- (CGFloat)progressForFrame:(CGFloat)frame {
  ...
  CGFloat progression = LOT_RemapValue(frame, self.leadingKeyframe.keyframeTime.floatValue, self.trailingKeyframe.keyframeTime.floatValue, 0, 1);
