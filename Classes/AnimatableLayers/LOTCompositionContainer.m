...
- (void)displayWithFloatFrame:(CGFloat)frame forceUpdate:(BOOL)forceUpdate {
    if (ENABLE_DEBUG_LOGGING) NSLog(@"-------------------- Composition Displaying Frame %f --------------------", frame);
    [super displayWithFloatFrame:frame forceUpdate:forceUpdate];
      
    CGFloat newFrame = (frame  - _frameOffset.floatValue) / self.timeStretchFloatFactor;
    if (_timeInterpolator) {
      newFrame = [_timeInterpolator floatValueForFloatFrame:newFrame];
    }
    for (LOTLayerContainer *child in _childLayers) {
        [child displayWithFloatFrame:newFrame forceUpdate:forceUpdate];
...
- (void)displayWithFrame:(NSNumber *)frame forceUpdate:(BOOL)forceUpdate {
  ...
  CGFloat newFrame = (frame.floatValue  - _frameOffset.floatValue) / self.timeStretchFactor.floatValue;
  if (_timeInterpolator) {
