...
@implementation LOTLayerContainer {
  ...
  NSNumber *_outFrame;
    ...
    CGFloat _inFloatFrame;
    CGFloat _outFloatFrame;
...
- (void)commonInitializeWith:(LOTLayer *)layer
  ...
  _outFrame = [layer.outFrame copy];
    _inFloatFrame = layer.inFrame.floatValue;
    _outFloatFrame = layer.outFrame.floatValue;
    
  _timeStretchFactor = [layer.timeStretch copy];
    _timeStretchFloatFactor = [layer.timeStretch floatValue];
  ...
  if (asset.imageName) {
    ...
    if ([asset.imageName hasPrefix:@"data:"]) {
      // Contents look like a data: URL. Ignore asset.imageDirectory and simply load the image directly.
        NSLog(@"asset.imageName -> %@", asset.referenceID);
        NSString *cacheKey = [self MD5ForUpper32Bate:asset.imageName];
        id<LOTImageCache> imageCache = [LOTCacheProvider imageCache] ?: [KWLOTDefaultCacheProvider defaultImageCache];
        if (imageCache) {
            if ([imageCache respondsToSelector:@selector(imageForKey:)]) {
                image = [imageCache imageForKey:cacheKey];
            }
            if (!image) {
                NSURL *imageUrl = [NSURL URLWithString:asset.imageName];
                NSData *imageData = [NSData dataWithContentsOfURL:imageUrl];
                image = [UIImage imageWithData:imageData];
                if ([imageCache respondsToSelector:@selector(setImage:forKey:)]) {
...
//    [self displayWithFrame:presentation.currentFrame];
      [self displayWithFloatFrame:presentation.currentFrame];
  }
}

- (void)displayWithFloatFrame:(CGFloat)frame {
    [self displayWithFloatFrame:frame forceUpdate:NO];
}

- (void)displayWithFloatFrame:(CGFloat)frame forceUpdate:(BOOL)forceUpdate {
    CGFloat newFrame = frame / self.timeStretchFloatFactor;
    if (ENABLE_DEBUG_LOGGING) NSLog(@"View %@ Displaying Frame %f, with local time %f", self, frame, newFrame);
    BOOL hidden = NO;
    if (_inFrame && _outFrame) {
      hidden = (frame < _inFloatFrame ||
                frame > _outFloatFrame);
    }
    self.hidden = hidden;
    if (hidden) {
      return;
...
//  [self displayWithFrame:frame forceUpdate:NO];
    [self displayWithFloatFrame:frame.floatValue forceUpdate:NO];
}

- (void)displayWithFrame:(NSNumber *)frame forceUpdate:(BOOL)forceUpdate {
    [self displayWithFloatFrame:frame.floatValue forceUpdate:forceUpdate];
